...
- 34 条正式视频已完成预处理、场景切分、ASR、OCR、VLM 和统一证据构建；
- 全分析校验通过：225 个场景、348 条语音片段、1,444 条 OCR 观测、225 条 VLM 场景描述，共 2,017 条证据，场景空文本为 0；
- 文本稠密索引已覆盖 225 个场景（Qwen3-Embedding-0.6B，1,024 维）；
- 直接视频视觉索引已覆盖 225 个场景（Qwen3-VL-Embedding-2B，2,048 维）；
...
暖机后单查询中位延迟为：文本混合 274.9 ms、直接视觉 258.4 ms、三路融合 528.4 ms。强制模式检查确认没有静默退回 BM25。完整逐问题结果保存在 `reports/video_retrieval_incremental_20260723.json`。
