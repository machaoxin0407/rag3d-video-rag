...
- AI 逐场景生成中英文摘要、产品、部件、动作、状态、安全文字、不确定性及时间边界预标注；
- R1 复核全部 AI 保留场景并修正错误，同时对 AI 拒绝场景执行 20% 固定分层抽查；
- R1 确认源清单产品类别是否需要修正，并对许可、隐私和安全风险作最终判断；
- 对模型文本的人工修改标记为 `corrected`，保留原始模型版本、提示词版本和修正原因；
- `rejected` 场景不进入论文测试真值，但保留在错误分析清单中。

正式评估集冻结前，应报告产品/动作/状态标签的 AI 修正率，以及拒绝抽查误拒率和 Wilson 95% 置信区间。模型生成文本只作为预标注，不得直接作为评估答案；由于只有一名复核人，不报告标注员间一致性。

## 下一阶段

1. 完成 25 场景的 AI 预标注与 R1 全量复核；
