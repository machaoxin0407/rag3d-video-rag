...
- Codex：机器预筛和证据整理，不计入人工审核；
- A：逐条打开来源页，核对作者、许可和授权范围；
- B：核对产品范围、步骤价值、隐私、人物和第三方素材；
- C：处理分歧、确定最终接受状态并冻结 train/dev/test。

一条视频只有在 A、B 均通过且 C 完成裁决后，才能设置 `final_decision=accept`。
